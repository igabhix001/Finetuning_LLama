...
def _build_system_prompt(with_rag=True):
    ...
    _base = (
        ...
        "## CRITICAL REASONING RULES (ANTI-TEMPLATE):\n"
        "1. DIFFERENT EVENTS = DIFFERENT DASHAS. Marriage, children, graduation, injuries CANNOT all happen in the same dasha period.\n"
        "   - Marriage = 7th cusp sub-lord activation\n"
        "   - Children = 5th cusp sub-lord activation\n"
        "   - Education = 4th cusp sub-lord activation\n"
        "   - Injuries = 6th cusp sub-lord activation\n"
        "   - Each event needs its OWN dasha period that activates the SPECIFIC houses.\n\n"
        "2. COMMON SENSE CHECK:\n"
        "   - First injury CANNOT be at age 28. Childhood injuries (age 5-15) are normal.\n"
        "   - Graduation is typically age 20-25, NOT age 28.\n"
        "   - Marriage + first child in same 2-month window is RARE — acknowledge if this appears.\n\n"
        "3. VARY YOUR RESPONSES. Do NOT use the same template structure for every answer:\n"
        "   - WRONG (template): 'Rito ji, your [X] likely occurred during [dasha] from [dates], when you were around [age] years old — [planet] as both mahadasha and antardasha lord signifies houses [Y,Z]...'\n"
        "   - RIGHT (varied): 'Jul 2014 during Rahu-Jupiter AD — 7th CSL activation. Sahi hai?'\n"
        "   - RIGHT (varied): 'Your 5th house activated in Saturn-Venus period (2021-2023). Child born around that time?'\n"
        "   - RIGHT (varied): 'Graduation completed during Jupiter period when you were 22-23 years old.'\n\n"
        "4. MATCH HOUSES TO EVENTS:\n"
        "   - If user asks about marriage, ONLY look at dashas that activate 7th cusp sub-lord houses.\n"
        "   - If user asks about children, ONLY look at dashas that activate 5th cusp sub-lord houses.\n"
        "   - If user asks about education, ONLY look at dashas that activate 4th cusp sub-lord houses.\n"
        "   - DO NOT give the same dasha for different life events unless the chart truly shows overlap.\n\n"
...
def _build_system_no_rag():
    ...
    return (
        ...
        "   - Education = 4th cusp sub-lord activation\n"
        "   - Injuries = 6th cusp sub-lord activation\n"
        "2. COMMON SENSE: First injury CANNOT be at age 28. Graduation is typically age 20-25.\n"
        "3. VARY RESPONSES. Do NOT use same template for every answer.\n"
        "4. MATCH HOUSES TO EVENTS: Marriage → 7th CSL houses only. Children → 5th CSL houses only.\n\n"
        ...
        if not is_hindi_question:
            ...
            if not _skip_deep_filter and _hindi_body_count >= 4:
                ...
                for _s in _sents:
                    ...
                    if _has_date or _hindi_density < 0.25:
